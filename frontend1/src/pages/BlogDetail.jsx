...
export const BlogDetail = () => {
  ...
  const fetchCommentsForPost = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5003/comments/${params.id}`,
        {
          headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5003/blog/${params.id}`,
          {
            headers: {
              authorization: `Bearer ${token}`,
            },
  ...
  async function handleAddComment() {
    ...
    try {
      const response = await axios.post(
        `http://localhost:5003/comment/${Blog._id}`,
        { content },
        {
          headers: {
            authorization: `Bearer ${token}`,
  ...
  async function handleDelete(postId) {
    try {
      const response = await axios.delete(
        `http://localhost:5003/comment/${postId}`,
        {
          headers: {
            authorization: `Bearer ${token}`,
          },
  ...
  async function handleFollow(postId) {
    ...
    try {
      const response = await axios.post(
        `http://localhost:5003/follow/${postId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
              ...
              >
                ...
                <Box mb={2}>
                  <Typography variant="body1">
                    {" "}
                    <strong>Author : {Blog.user_name}</strong>
                    {userId !== Blog.authorId && (
                      ...
                      >
                        {userData?.following.includes(Blog.authorId)
                          ? "Following"
                          : "Follow"}
                      </Button>
                    )}
                ...
                <Box mt={2} mb={3} display={"flex"}>
                  {Blog && (
                    <>
                      <IconButton
                        sx={{
                          color: Blog.likes.includes(userId)
                            ? "green"
                            : "inherit",
                        }}
                      ...
                      <IconButton
                        ...
                        sx={{
                          color: Blog.dislikes.includes(userId)
                            ? "red"
                            : "inherit",
                        }}
                      >
                    ...
                    return (
                      ...
                      >
                        ...
                        <CardMedia
                          ...
                          image={
                            blog.image ||
                            "/static/images/cards/default-image.jpg"
                          }
                        />
                        <CardContent>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText sx={{ fontSize: "20px" }}>
            you have to
            <Link style={{ color: "#90CAF9", paddingLeft: "5px" }} to="/login">
              login
            </Link>{" "}
            or
            <Link
              style={{ color: "#90CAF9", paddingLeft: "5px" }}
              to={"/signup"}
            >
              Signup
            </Link>
